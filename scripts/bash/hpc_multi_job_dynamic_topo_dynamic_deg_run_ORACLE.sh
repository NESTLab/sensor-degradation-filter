...
sed -i -E "s/(DYNAMIC_QUEUE_WINDOW_SIZE=).*/\1${DYNAMIC_QUEUE_WINDOW_SIZE}/" ${SBATCH_SCRIPT_TEMPLATE}
...
for ((i = 0; i < ${#TRUE_DEG_DRIFT[@]}; i++)); do
    ...
    for ((j = 0; j < ${#DENSITY[@]}; j++)); do
        ...
        sed -i -E "s/(WALL_POSITION=).*/\1${WALL_POSITION[j]}/" ${SBATCH_SCRIPT_TEMPLATE}
